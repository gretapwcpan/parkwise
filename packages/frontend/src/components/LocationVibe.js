...
const LocationVibe = ({ location, onClose, onFindSimilar, onHashtagClick, onLocationClick }) => {
  ...
  const analyzeLocation = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/vibe/analyze`, {
        method: 'POST',
  ...
  const findSimilarByHashtags = async (hashtags) => {
    ...
    try {
      const API_BASE_URL = process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000';
      const response = await fetch(`${API_BASE_URL}/api/vibe/similar`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          hashtags,
